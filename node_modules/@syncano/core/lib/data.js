...
class Data extends _queryBuilder2.default {
  url(id, apiVersion) {
    ...
    let url = `${this._getInstanceURL(instanceName, apiVersion)}/classes/${className}/objects/${id ? id + '/' : ''}`;
  ...
  async list() {
    debug('list');
    const self = this;
    const urls = [this.url(null, 'v3')].concat(this.queries.map(query => (this._query.query = query) && this.url(null, 'v3')));
    debug('urls', urls);
    const uniqueIds = [];
  ...
  _mapFieldsForSingleItem(item, fields) {
    return Object.keys(fields).reduce((all, key) => {
      ...
      const itemField = (0, _lodash4.default)(item, itemFieldKey);
      ...
      if (Array.isArray(itemField) && typeof itemField[0] === 'object' && itemField[0] !== null) {
        itemField.forEach((arrItem, i) => {
          ...
          (0, _lodash2.default)(all, mappedPath, (0, _lodash4.default)(item, path));
        });
      } else {
        (0, _lodash2.default)(all, fields[key] || key, (0, _lodash4.default)(item, key));
      }
