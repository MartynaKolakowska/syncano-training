...
process.on('unhandledRejection', (reason, p) => {
  if (reason.name === 'RequestError' && reason.status === 403) {
    if (reason.errors.detail === 'No such API Key.') {
      const errorMessage = `API key from your config file is not valid. Use ${_chalk2.default.cyan('npx s login')} to log in once again.`;
...
class ErrorResponse {
  ...
  static handleRequestError(err, name, contextName) {
    if (err.status === 404) {
      ...
      (0, _printTools.echo)();
      (0, _printTools.echo)(`Did you run ${_chalk2.default.green('npx s deploy')} command?`);
      return;
    }
