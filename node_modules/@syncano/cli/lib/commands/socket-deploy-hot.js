...
class SocketDeployCmd {
  ...
  run([socketName, cmd]) {
    ...
    return _asyncToGenerator(function* () {
      ...
      (0, _printTools.echo)(2)(`🔥 ${_chalk2.default.grey(' Hot deploy started')} ${_chalk2.default.dim('(Hit Ctrl-C to stop)')}`);
      (0, _printTools.echo)();
      ...
      if (socketName) {
        ...
        _this.socketList = yield _this.Socket.list(socketName);
  ...
  deploySocket(socket, config) {
    ...
    return _asyncToGenerator(function* () {
      ...
      try {
        const updateEnv = !_this3.firstRun[socket.name];
        const updateStatus = yield socket.update({ config, updateEnv });

        spinner.stop();
        SocketDeployCmd.printUpdateSuccessful(socket.name, updateStatus, deployTimer);
        yield updateEnds();
        _this3.firstRun[socket.name] = true;
      } catch (err) {
        debug(err);
        spinner.stop();
