...
class SocketCreate {
  ...
  run([socketNameArg]) {
    ...
    return _asyncToGenerator(function* () {
      ...
      (0, _printTools.echo)();
      try {
        const spinner = new _spinner.SimpleSpinner((0, _printTools.p)(2)('Creating Socket...')).start();
        const socket = yield _this.Socket.create(socketName, template);
        yield socket.build();
        spinner.stop();
        spinner.succeed((0, _printTools.p)(2)(`Your Socket configuration is stored at ${_chalk2.default.cyan(socket.getSocketPath())}`));
        (0, _printTools.echo)();
      } catch (err) {
        debug(err);
