...
const setup = (() => {
  var _ref = _asyncToGenerator(function* () {
    ...
    _program2.default.command('add <path>').group('Hosting').description('Create a new hosting').option('-n, --hosting-name <name>', 'Name of the hosting').option('--browser-router-on', 'Turn on browser router for a hosting').option('--browser-router-off', 'Turn off browser router for a hosting').option('--dont-sync', 'Do not sync files after adding hosting').option('--sync', 'Sync files after adding hosting').option('--without-cname', 'No CNAME').option('-c, --cname <cname>', 'CNAME').action((() => {
    ...
    _program2.default.command('sync <name>').group('Hosting').description('Publish your local hosting files').option('-d, --no-delete', 'Update and add files only (do not delete files)').option('-s, --socket <name>', 'socket').action((() => {
    ...
    _program2.default.command('config <name>').group('Hosting').description('Configure hosting parameters').option('-c, --cname <domain_name>', 'add CNAME to hosting').option('-d, --remove-cname <domain_name>', 'remove CNAME from hosting').option('-b, --browser-router <true|false>', 'turn on/off the BrowserRouter support').action((() => {
